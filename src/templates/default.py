"""极简白模板 — 白色背景、宋体标题，适合日常阅读和打印。"""

import html

NAME = "极简白"
DESCRIPTION = "白色极简风格，适合日常阅读和打印"
...
def render(group_name, date_str, stats, topics, user_titles,
    ...
    for idx, topic in enumerate(topics, 1):
        ...
        card_html = (
            ...
            f'<div class="topic-head"><span>{idx}</span><strong>{_esc(topic.get("topic", "未知话题"))}</strong></div>'
            f'<div class="meta">{_esc(time_range + " / " if time_range else "")}参与者：{_esc(contributors)}</div>'
    ...
    for item in user_titles[:top_users_count]:
        ...
        user_cards.append(
            ...
            '<div>'
            f'<div class="user-name">{_esc(name)}</div>'
            f'<div class="badge">{_esc(title)}</div>'
            f'<p>{_esc(reason)}</p>'
            '</div></div>'
        )
    ...
    if quote and quote.get("text"):
        quote_html = (
            '<div class="quote"><p class="quote-text">「' + _esc(quote["text"]) + '」</p>'
            '<p class="quote-source">—— ' + _esc(quote.get("source", "佚名")) + '</p></div>'
        )
...
<body>
  <div class="page">
    <header>
      <h1>群聊日常分析报告</h1>
      <div class="date">{_esc(group_name)} / {date_str}</div>
    </header>
    <section>
      <h2>基础统计</h2>
      <div class="stats">
